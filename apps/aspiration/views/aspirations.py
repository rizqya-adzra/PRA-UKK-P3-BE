...
class AspirationProgressListCreateView(generics.ListCreateAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not queryset.exists():
             return response_success(message="Belum ada data Progres Aspirasi", data=[])

        serializer = self.get_serializer(queryset, many=True)
        return response_success(message="List semua data Progres Aspirasi", data=serializer.data)
